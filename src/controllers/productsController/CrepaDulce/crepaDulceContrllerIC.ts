...
class CrepaDulceIngredienteComplementarioController {
    ...
    public async create(req: Request, res: Response): Promise<void> {
        if(req.headers['authorization'] === undefined){
          ...
          await pool.promise().query('INSERT INTO cdic SET ?', {ingrediente_com:ingrediente, tipo:tipo, inventario: req.body.inventario, adminId: adminId})
          .then((result:any) => {
          ...
          .then((sucursales:any) => {
            sucursales[0].forEach(async (sucursal:any) => {
              ...
              await pool.promise().query('INSERT INTO cdice SET ?', [registro])
              .then(() => {
    ...
    public async delete(req: Request, res: Response){
        ...
        await pool.promise().query('DELETE FROM cdice WHERE product_id = ? AND adminId = ?', [ingredienteId, adminId])
        .then(() => {
...
const adminId = decodedToken.id
        ...
        await pool.promise().query(query, [ingrediente1, ingredienteId, adminId])
          ...
          // Actualizar ventas y existencias de ingredientes complementarios
              ...
              await pool.promise().query('UPDATE cdice SET ? WHERE product_id = ? AND adminId = ?', [registro, ingredienteId, adminId])
              .then(() => {
